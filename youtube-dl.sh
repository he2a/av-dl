...
function getAudio () {
	...
	youtube-dl -f 'bestaudio' --extract-audio --audio-format wav -o "temp_ytdl/audio.wav" $url
...
function trmThumb () {
	...
	ffmpeg -y -hide_banner -loglevel error -i 'temp_ytdl/thumb001.png' -vf "$crop,crop=w='min(iw\,ih)':h='min(iw\,ih)',setsar=1" -q:v 2 'temp_ytdl/cover.jpg'
}
	...
	v | V | video | Video | VIDEO | 2)
		...
		if [ "$embedsubs" = true ]; then
			if [ "$autosubs" = true ]; then
				...
				youtube-dl -f 'bestvideo+bestaudio/best' --merge-output-format mkv --write-sub --write-auto-sub --sub-lang en --embed-subs -o "$title.%(ext)s" $url
			else
				INFO "Writing video file."
			...
			INFO "Writing video file."
			youtube-dl -f 'bestvideo+bestaudio/best' --merge-output-format mkv -o "$title.%(ext)s" $url
		fi
	;;
